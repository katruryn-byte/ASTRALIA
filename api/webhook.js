...
const { CATALOGO } = require('./pagamento');
...
async function processarAprovacao(sessionId, paymentId, redisUrl) {
  ...
  // Produto vem da sessao (foi gravado em pagamento.js); fallback para o Personalizado
  const produto = sessionObj.produto || PRODUTO_PADRAO;
  const meta = CATALOGO[produto];

  if (sessionObj.dados) {
    try {
      const reg = await registrarCompra(sessionObj.dados, produto);
  ...
  // Divergencia:
  //   ISCA  -> nao enfileira; /api/leitura sera chamado inline pelo front
  //   PREMIUM -> empurra na fila do produto, o n8n consome quando subir
  if (meta && !meta.isca) {
